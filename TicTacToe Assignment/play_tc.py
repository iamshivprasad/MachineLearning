...
class PlayTicTacToe():
  ...
  def __init__(self):
    ...
    with open('Submission1/Policy.pkl', "rb") as policy_file:
  ...
  def start(self):
    ...
    self.state, reward, is_terminal = self.env.step(self.state, first_action, train_mode=False)

    while not is_terminal:
      agent_action = self.get_agent_action()

      self.state, reward, is_terminal = self.env.step(self.state, agent_action, train_mode=False)
